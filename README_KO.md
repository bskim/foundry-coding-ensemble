...
점수화하고 동점일 때만 LLM 심판이 타이브레이커로 개입)과 비용 인지 라우팅(후보를 저렴한
순으로 조회해 처음 깨끗한 후보를 채택). 교차 모델 분산을 드러내도록 큐레이션한 10개 SWE-bench
Verified 인스턴스(disc10 세트)에서, 실행 기반 선택은 10/10을 풀어 최고 단일 오픈 모델(glm-5.1,
9/10)과 단일 모델 SOTA 베이스라인(gpt-5.4, 7/10)을 모두 넘어섭니다 - 앙상블이 최고 단일
모델을 처음으로 이긴 구성입니다. 인스턴스별 해결 매트릭스와 정직한 단서(큐레이션된 변별 세트,
소규모, 심판 분산)까지 담은 전체 결과는
[reports/fusion-ensemble-report_ko.md](reports/fusion-ensemble-report_ko.md)를 참고하세요.

단서: 이것은 의도적으로 간략화한 에이전트 하니스입니다. 체감 품질은 - 단일 에이전트도,
앙상블도 - 실제 코딩 에이전트의 클라이언트 사이드 하니스(Claude Code, Codex, Copilot, Aider)
성숙도에 따라 크게 달라지므로, 수치는 절대 역량이 아니라 '이 하니스 안에서의 상대 비교'로
읽어야 합니다.
...
없이 패널을 고르는 held-out 분리, 그리고 모델이 본 적 없는 비공개 과제에서의 검증이 필요합니다.
...
그 단일 모델 후보들을 앙상블로 묶는 하니스(harness)를 더합니다. 두 리포지토리는 같은 gross
비용 방식과 같은 공식 SWE-bench Verified Docker 하니스를 공유하지만, 채점하는 인스턴스
서브셋이 다릅니다 - foundry-model-benchmark는 자체 선정 세트를, 여기 수치는 disc10 세트를
평가하므로 - 두 리포지토리의 모델별 수치를 직접 비교할 수는 없습니다.
...
100%는 '이 세트에서의 상한 신호'이지 벤치마크급 해결률이 아닙니다. 비용은 gross 기준(캐시
할인 없음)이며, 달러 수치를 신뢰하기 전 [harness/pricing.json](harness/pricing.json)을 본인
요율로 맞춰 두세요. gpt-5.5와 opus-4.8은 쿼터 대기 중인 최신 SOTA 베이스라인으로, 같은 disc10
세트에 solo 베이스라인으로 추가될 예정입니다.
...
오픈 모델(glm-5.1, 9/10)과 독점 SOTA 베이스라인(gpt-5.4, 7/10)을 모두 이기고, 비용 인지
...
그렇다는 것은 아닙니다. 남은 일은 비큐레이션 표본에서 평가 규모를 키우고, 최신 SOTA
베이스라인(gpt-5.5, opus-4.8, 쿼터 대기)을 같은 disc10 세트에 solo로 추가한 뒤 오픈/독점 혼합
패널을 시험하는 것입니다. 실무에서는 매 과제를 채점하지 않고
사람의 정성 리뷰가 대신하며, SWE-bench는 방법 검증용 하니스로만 둡니다. 자세한 내용은
